...
func (r *Router) routeConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) error {
	...
	// Fast-path: check direct inbound→outbound binding (dynamic proxies)
	r.bindingsMu.RLock()
	boundTag, hasBound := r.inboundBindings[metadata.Inbound]
	r.bindingsMu.RUnlock()
	if hasBound {
		selectedOutbound, loaded := r.outbound.Outbound(boundTag)
		if !loaded {
			return E.New("bound outbound not found: ", boundTag)
		}
		...
		for _, tracker := range r.trackers {
			conn = tracker.RoutedConnection(ctx, conn, metadata, nil, selectedOutbound)
		}
...
func (r *Router) routePacketConnection(ctx context.Context, conn N.PacketConn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) error {
	...
	if hasBound {
		...
		for _, tracker := range r.trackers {
			conn = tracker.RoutedPacketConnection(ctx, conn, metadata, nil, selectedOutbound)
		}
		if metadata.FakeIP {
